...
const AllMovies = () => {
  ...
  const handleGenreClick=(genreId)=>{
    ...
    const handleYearChange=(year)=>{
    const filteredByYear= data.filter(movie=> movie.year===+year);
    dispatch(setFilteredMovies(filteredByYear))
  }
  ...
  return (
    <div className="mb-24 grid grid-col-1 sm:grid-cols-2 md:grid-col-3 lg:grid-cols-4 gap-4 -translate-y-[5rem]">
      <>
        <section>
          <div
            className=" relative h-[50rem] w-screen mb-10 flex items-center justify-center bg-cover "
            style={{ backgroundImage: `url(${banner})` }}
          >
            <div className="absolute inset-0 bg-gradient-to-b from-gray-700 to-black opacity-60"></div>
            <div className="relative z-10 text-center text-white mt-[10rem] ">
              <h1 className="text-8xl font-bold mb-4"> The Movie Hub</h1>
              <p className="text-2xl">
                Cinematic Odyssey: Unveiling the Magic of Movies
              </p>
            </div>
            <section className="absolute -bottom-[5rem] mb-16">
              <input
                type="text"
                className="w-[100%] h-[5rem] border px-10 outline-none rounded "
                placeholder="Search Movie"
                ...
                onChange={handleSearchChange}
              ></input>
              <section className="sorts-container mt--[2rem] ml-[10rem] w-[30rem] mt-4">
                <select
                ...
                <select
                  className="border p-2 rounded text-black ml-2"
                  value={moviesFilter.selectedSort}
                  onChange={(e)=>handleSortChange(e.target.value)}
                >
                  <option value="">Sort By</option>
      ...
      <section className="mt-[60rem] w-screen flex justify-center items-center flex-wrap ">
        {filteredMovies?.map((movie) => (
          <MovieCard key={movie._id} movie={movie} />
        ))}
      </section>
